...
int32_t start, end, fnd_sub_start, fnd_sub_end, use_console_start,
		use_console_end;
...
void use_console() {
	...
	if (ring_buf_pop(&rbuf, &indata)) {
		if (indata == '\r' || indata == '\n') {
			use_console_start = count; //start count
...
//							&& hex1 <= ADDR_FLASH_PAGE_127_END
			...
			use_console_end = count; //end count
			use_console_run = (double) (use_console_end - use_console_start)/10;
			PRINTF_DEBUG("use_console running time : %.3fms\r\n",
					use_console_run);
		} else {
			if (rx_frame_cnt == 0) {
				buf_clear(&rbuf);
